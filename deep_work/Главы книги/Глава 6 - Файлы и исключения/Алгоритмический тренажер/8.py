# Программа меняет оценку студента
...
def main():
    ...
    # Поиск записи
    search = input('Какому студенту желаете изменить балл? ')

    # Запись нового значения баллов
    new_ball = int(input('Введите кол-во баллов: '))
    ...
    while name != '':
        ...
        # Условие if-else для проверки данных на изменение записи
        if name == search:
            # Поместить запись во временный файл
            temp_file.write(f'{name}\n')
            temp_file.write(f'{new_ball}\n')

            # Назначить флагу значение True
            found = True
        else:
            # Записать данные изменяя
            temp_file.write(f'{name}\n')
