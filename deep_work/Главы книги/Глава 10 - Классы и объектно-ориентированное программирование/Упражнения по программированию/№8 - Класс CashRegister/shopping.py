...
def main():
    ...
    while again.lower() == "д":
        ...
        logging.debug(
            "Происходит добавление объекта класса RetailItem в объект класса CashRegister..."
        )
...
def get_menu_choice(r_obj):
    ...
    for index in range(len(r_obj)):
        print(
            f"{index + 1}. {r_obj[index].get_description()} - {r_obj[index].get_price()}$"
        )

    logging.debug("Пользователь выбирает пункты меню.")
    # Получить выбранный пользователем пункт
    choice = int(
        input(f"Введите номер товара который хотите купить (1 - {len(r_obj)}): ")
    )
    ...
    while choice < 1 or choice > len(r_obj):
        choice = int(
            input(f"Введите номер товар который хотите купить (1 - {len(r_obj)}): ")
        )
